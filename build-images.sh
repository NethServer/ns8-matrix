...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-matrix; then
    ...
    buildah from --name nodebuilder-matrix -v "${PWD}:/usr/src:Z" docker.io/library/node:24.14.0-slim
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:14.22-alpine ghcr.io/element-hq/synapse:v1.149.1 ghcr.io/element-hq/element-web:v1.12.12 ghcr.io/cinnyapp/cinny:v4.10.2 ghcr.io/nethesis/matrix2acrobits:0.0.4" \
    "${container}"
# Commit the image
